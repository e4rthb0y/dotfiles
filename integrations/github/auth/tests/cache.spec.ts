...
Deno.test('FileCacheProvider - set and get encrypted', async () => {
    ...
    const cache = new FileCacheProvider<{ token: string }>(cachePath, TEST_SEED, 0)
    ...
    assertNotEquals(rawContent.includes('test-token'), true, "Cache file should not contain plain text data")
...
Deno.test('FileCacheProvider - expired data', async () => {
    const tempDir = await Deno.makeTempDir()
    const cachePath = join(tempDir, 'cache.json')
    const cache = new FileCacheProvider<{ token: string }>(cachePath, TEST_SEED)

    const data = { token: 'expired-token' }
...
Deno.test('FileCacheProvider - missing file', async () => {
    const cache = new FileCacheProvider<{ token: string }>('non-existent.json', TEST_SEED)
    const cachedData = await cache.get()
    assertEquals(cachedData, null)
})
...
Deno.test('FileCacheProvider - wrong seed cannot decrypt', async () => {
    ...
    const cache1 = new FileCacheProvider<{ token: string }>(cachePath, TEST_SEED, 0)
    const cache2 = new FileCacheProvider<{ token: string }>(cachePath, "wrong-seed", 0)

...
VM_SRC="$DOTFILES_DIR/taskw/hooks/value-matrix.py"
TASK_HOOKS_DIR="$HOME/.task/hooks"

if [ -f "$VM_SRC" ]; then
    chmod +x "$VM_SRC"

    link_file "$VM_SRC" "$TASK_HOOKS_DIR/on-add.value_matrix.py"
    link_file "$VM_SRC" "$TASK_HOOKS_DIR/on-modify.value_matrix.py"
